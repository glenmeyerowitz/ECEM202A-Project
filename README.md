...
Much of the code used for this project was from a tutorial put together by Dr. Adrian Rosebrock [7]. Small modifications to the code were made, as necessary, to allow it to work for my requirements. In this tutorial, Dr. Rosebrock uses a trained CNN to identify natural disasters using a pretrained VGG16 network and adding additional training images for the specific area of interest. I was able to develop a custom dataset and use much of the same code to train a VGG16 network on a new dataset and then perform inferences and label video data. The efficient use of existing code repositories allowed me to best utilize my time on the novel portions of this project.
